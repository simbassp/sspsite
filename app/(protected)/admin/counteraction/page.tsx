...
import { counteractionBadgeStyle, splitCategoryLabels } from "@/lib/catalog-badges";
import {
...
import {
  deleteCounteractionItem,
  fetchCounteractionItems,
  reorderCounteractionItems,
  saveCounteractionItem,
...
export default function AdminCounteractionPage() {
  ...
  const categoryOptions = useMemo(() => buildCounteractionCategoryOptions(customCategories), [customCategories]);
  const isKnownCategory = Boolean(findCanonicalCounteractionCategory(draft.category, categoryOptions));
  ...
  const visibleItems = useMemo(() => {
    ...
    return orderedItems.filter((item) => itemMatchesCounteractionCategory(item.category, categoryFilter));
    ...
    try {
      ...
      if (res.ok && payload.ok) {
        ...
        if (payload.migrationRequired) {
          ...
          setCustomCategories(buildCounteractionCategoryOptions(local).filter((c) => !isBuiltinCounteractionCategory(c)));
  ...
  const saveCustomCategory = async (rawLabel: string) => {
    ...
    if (isBuiltinCounteractionCategory(label)) {
      const canonical = findCanonicalCounteractionCategory(label, categoryOptions) || label;
    ...
    setCategoryHint("");
    try {
      const res = await fetch("/api/admin/counteraction/categories", {
        method: "POST",
      ...
      if (res.ok && payload.ok) {
        ...
        setCustomCategories((prev) =>
          buildCounteractionCategoryOptions([...prev, saved]).filter((c) => !isBuiltinCounteractionCategory(c)),
        );
        writeLocalCustomCategories(
          buildCounteractionCategoryOptions([...customCategories, saved]).filter(
            (c) => !isBuiltinCounteractionCategory(c),
          ),
        );
      ...
      if (payload.error === "migration_required_counteraction_category_presets") {
        const next = buildCounteractionCategoryOptions([...customCategories, label]).filter(
          (c) => !isBuiltinCounteractionCategory(c),
        );
  ...
  const deleteCustomCategory = async (label: string) => {
    if (isBuiltinCounteractionCategory(label)) return;
    setCategoryBusy(true);
    try {
      const res = await fetch(`/api/admin/counteraction/categories?label=${encodeURIComponent(label)}`, {
        method: "DELETE",
      });
      const payload = (await res.json()) as { ok?: boolean; error?: string; message?: string };
      const next = customCategories.filter(
        (c) => c.trim().toLowerCase() !== label.trim().toLowerCase() && !isBuiltinCounteractionCategory(c),
      );
      if ((res.ok && payload.ok) || payload.error === "migration_required_counteraction_category_presets" || !res.ok) {
        setCustomCategories(next);
        writeLocalCustomCategories(next);
        ...
        setCategoryHint(payload.error === "migration_required_counteraction_category_presets" ? "Удалено в этом браузере." : "");
        return;
      }
  ...
  const persistOrder = useCallback(async (nextOrdered: CatalogItem[]) => {
    ...
    setReorderMessage("");
    try {
      await reorderCounteractionItems(withOrder.map((item) => item.id));
    ...
    (from: number, to: number, opts?: { persist?: boolean }) => {
      ...
      visibleIdsRef.current =
        categoryFilter === "all"
          ...
          : withOrder
              .filter((item) => itemMatchesCounteractionCategory(item.category, categoryFilter))
              .map((item) => item.id);
  ...
  const onEdit = (item: CatalogItem) => {
    setMessage("");
    const known = Boolean(findCanonicalCounteractionCategory(item.category, categoryOptions));
  ...
  return (
    <section>
      ...
      <p className="page-subtitle">Категории, порядок карточек и редактирование — как у ТТХ БПЛА.</p>
          ...
          <div className="form" style={{ marginTop: 10 }}>
            ...
            <div className="chips" style={{ marginBottom: 8 }}>
              {categoryOptions.map((option) => {
                const selected =
                  !categoryModeOther && findCanonicalCounteractionCategory(draft.category, [option]) === option;
                const canDelete = !isBuiltinCounteractionCategory(option);
                return (
                  <span
                    key={option}
            ...
            <h3 style={{ marginTop: 4 }}>Параметры (до 8 строк)</h3>
      ...
      <div style={{ marginTop: 16 }}>
        ...
        <p className="page-subtitle" style={{ marginBottom: 8 }}>
          Перетащите за ⋮⋮ или кнопками ↑↓. Порядок сразу сохраняется и так же на странице противодействия.
        </p>
        ...
        <div className="chips" style={{ marginBottom: 10 }}>
          ...
          {categoryOptions.map((category) => {
            const count = orderedItems.filter((item) =>
              itemMatchesCounteractionCategory(item.category, category),
            ).length;
            return (
              <button
                key={category}
        ...
        {!isLoading && !loadError && !visibleItems.length && (
          <p className="page-subtitle">
            {categoryFilter === "all"
              ? "Пока нет карточек противодействия."
