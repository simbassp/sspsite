...
import { isBuiltinUavCategory, normalizeUavCategoryLabel } from "@/lib/uav-categories";
...
/** Список пользовательских категорий — доступен любому авторизованному. */
export async function GET() {
  ...
  try {
    ...
    const custom = (q.data || [])
      ...
      .filter((label) => label && !isBuiltinUavCategory(label));
...
export async function POST(request: Request) {
  const session = await getServerSession();
  if (!session || !canManageUav(session)) {
  ...
  if (isBuiltinUavCategory(label)) {
    return Response.json({ ok: true, label: label, alreadyBuiltin: true });
  }

  try {
    const supabase = getServerSupabaseServiceClient();
    const existing = await supabase.from("uav_category_presets").select("id,label");
    if (existing.error) {
      if (isMissingRelationError(existing.error.message)) {
        return Response.json(
          {
            ok: false,
            error: "migration_required_uav_category_presets",
            message: "Нужна миграция uav_category_presets в Supabase.",
          },
          { status: 500 },
        );
      }
    ...
    const dup = (existing.data || []).find(
      (row) => normalizeUavCategoryLabel(String(row.label || "")) === normalizeUavCategoryLabel(label),
    );
    if (dup) {
      return Response.json({ ok: true, label: String(dup.label), alreadyExists: true });
    }

    const ins = await supabase.from("uav_category_presets").insert({ label }).select("label").single();
    if (ins.error) {
...
export async function DELETE(request: Request) {
  ...
  try {
    const supabase = getServerSupabaseServiceClient();
    const rows = await supabase.from("uav_category_presets").select("id,label");
    if (rows.error) {
    ...
    const del = await supabase.from("uav_category_presets").delete().eq("id", target.id);
