...
export async function POST(request: Request) {
  ...
  if (new Set(orderedIds).size !== orderedIds.length) {
    return Response.json({ ok: false, error: "orderedIds_duplicate" }, { status: 400 });
  }

  try {
    const supabase = getServerSupabaseServiceClient();

    const existingQ = await supabase.from("catalog_items").select("id").eq("kind", "uav").in("id", orderedIds);
    if (existingQ.error) {
      if (isMissingColumnError(existingQ.error.message)) {
        return Response.json(
          { ok: false, error: "migration_required_sort_order", message: "Нужна миграция sort_order для catalog_items." },
          { status: 500 },
        );
      }
    ...
    const updates = orderedIds.map((id, index) =>
      supabase.from("catalog_items").update({ sort_order: index }).eq("id", id).eq("kind", "uav"),
    );
